...
import com.member.service.COMInformationService;
...
public class MainClasUseXmls {
	...
	public static void main(String[] args) {
		...
		// 각자 맞는 xmlApplicationContext 생성
		GenericXmlApplicationContext gtx_dao = new GenericXmlApplicationContext("classpath:appCtx1.xml");
		GenericXmlApplicationContext gtx_jdbc = new GenericXmlApplicationContext("classpath:appCtx2.xml");
		GenericXmlApplicationContext gtx_service = new GenericXmlApplicationContext("classpath:appCtx3.xml");
		
		//xml파일 String배열을 만든 뒤 xmlApplicationContext를 생성 할 수도있음
		String[] appCtxs = {"classpath:appCtx1.xml", "classpath:appCtx2.xml", "classpath:appCtx3.xml"};
		GenericXmlApplicationContext gtx_all = new GenericXmlApplicationContext(appCtxs);
		
		COMInformationService cis = gtx_service.getBean("infoService", COMInformationService.class);
		
		cis.outputEMSInformation();


		
		
		StudentRegisterService registerService = gtx_dao.getBean("registerService",
				StudentRegisterService.class);
		for (int j = 0; j < sNums.length; j++) {
			StudentDTO student = new StudentDTO(sNums[j], sIds[j], sPws[j], sNames[j], 
					sAges[j], sGenders[j], sMajors[j]);
			registerService.register(student);
		}

		
		StudentModifyService modifyService = gtx_dao.getBean("modifyService",
				StudentModifyService.class);
		
		modifyService.modify(new StudentDTO("H39lesvj7544vf89", "deer", "00000", "melissa", 
				26, "W", "Vocal Music"));

		
		StudentSelectService selectService = gtx_dao.getBean("selectService",
				StudentSelectService.class);
		
		StudentDTO modifiedStudent = selectService.select("H39lesvj7544vf89");
...
//		StudentAllSelectService allSelectService = assembler.getAllSelectService();
		...
		StudentAllSelectService allSelectService = gtx_dao.getBean("allSelectService",
				StudentAllSelectService.class);
		...
		while(true) {
			...
			if(str.equals("2")) {
				...
				str = scanner.next();
				StudentDTO student = selectService.select(str);
				System.out.print("sNum:" + student.getsNum() + "\t");
				System.out.print("|sId:" + student.getsId() + "\t");
				System.out.print("|sPw:" + student.getsPw() + "\t");
				System.out.print("|sName:" + student.getsName() + "\t");
